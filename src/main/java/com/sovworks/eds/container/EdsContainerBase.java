...
public abstract class EdsContainerBase implements Closeable {
	...
	public static FileSystem loadFileSystem(RandomAccessIO io, boolean isReadOnly) throws IOException, Exception {
	/*
		if (ExFat.isExFATImage(io)) {
			if (ExFat.isModuleInstalled())
				return new ExFat(io, isReadOnly);
			if (ExFat.isModuleIncompatible())
				throw new Exception("Please update the exFAT module.");
			throw new Exception("Please install the exFAT module");
		}
*/
	...
	public FileSystem getEncryptedFS() throws IOException, Exception {
	...
	public RandomAccessIO initEncryptedFile(boolean isReadOnly) throws IOException {
		...
		return allowLocalXTS()
				? new LocalEncryptedFileXTS(_pathToContainer.getPathString(), isReadOnly,
						_layout.getEncryptedDataOffset(), (XTS) enc)
				: new EncryptedFileWithCache(_pathToContainer, isReadOnly ? AccessMode.Read : AccessMode.ReadWrite,
						_layout);
	}

	public synchronized FileSystem getEncryptedFS(boolean isReadOnly) throws IOException, Exception {
		if (_fileSystem == null) {
	...
	protected boolean tryLayout(RandomAccessIO containerFile, byte[] password, boolean isHidden)
	...
	protected boolean tryLayout(ContainerFormatInfo cf, RandomAccessIO containerFile, byte[] password, boolean isHidden)
			throws IOException, ApplicationException {
		if (isHidden && !cf.hasHiddenContainerSupport())
			return false;
		// Logger.debug(String.format("Trying %s container format%s",
		// cf.getFormatName(), isHidden ? " (hidden)" : ""));
	...
	protected boolean allowLocalXTS() {
		return _pathToContainer instanceof StdFsPath && _layout.getEngine() instanceof XTS
